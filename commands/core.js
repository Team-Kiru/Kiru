...
exports = {
  settings: {
    ...
    run: (client, message) => {
      const local = client.extraFunction.getLocalizedCommand(
        client,
        message,
        'settings'
      )
      const args = message.content
        .replace(
          client.extraFunction.getPrefix(client, message) + 'settings',
          ''
        )
        .trim()
        .split(' ')
      if (message.member.permissions.has('MANAGE_GUILD')) {
        if (
          ...
          const embed = {
            ...
            color: 8311585,
            footer: {
              text: local.footer_title.replace(
                '{user}',
            ...
            fields: [
              ...
              },
              {
                name: 'OnJoin',
                value:
                  'Want Kiru to welcome new members into the server? Or give a new role? This is the place!'
              },
              {
                name: 'OnLeave',
                value:
                  'Want Kiru to make a comment when people leave? Use this!'
              }
            ]
          }
          ...
          switch (args[0].toLowerCase()) {
            case local.prefix:
              if (args[1] === undefined) return
                ...
                client.data.put(
                  'serverSpecificSettings.' + message.guild.id + '.prefix',
                  undefined
                )
                client.data.save()
              ...
              break
            case 'onleave':
              onMember(client, message, args, 'leave')
              break
            case 'onjoin':
              onMember(client, message, args, 'join')
              break
          }
        }
      } else {
        message.reply(local.cant_change_settings)
      }
    }
  },
  language: {
    ...
    run: (client, message, args) => {
      let localization = JSON.parse(
        require('fs').readFileSync(
          './commands/localization/generic.json',
          'utf8'
        )
      )
      if (args.length === 0) {
        ...
        Object.keys(localization.languages).forEach(lang => {
          msg +=
            '\n' +
            localization.languages[lang].replace(
              '{prefix}',
              client.extraFunction.getPrefix(client, message)
            )
        })
        ...
        if (args[0] === 'server') {
          client.data.put(
            'serverSpecificSettings.' + message.author.id + '.language',
            args[1]
          )
          client.data.save()
          message.reply(
            client.extraFunction.getLocalizedCommand(
              client,
              message,
              'language'
            )['ok']
          ...
          client.data.put(
            'userSpecificSettings.' + message.author.id + '.language',
            args[0]
          )
          client.data.save()
          message.reply(
          ...
          message.reply("That's not a proper language. :c")
        }
      }
    }
  },
  about: {
    info: (client, message) => {
      let local = client.extraFunction.getLocalizedCommand(
        client,
        message,
        'about'
      )
    ...
    run: (client, message) => {
      ...
      embed.fields.push({
        ...
        value: local.command.versionText.replace(
          '{version}',
      ...
      embed.fields.push({
        ...
        value: local.command.specialThanksUnder.replace(
          '{bot.name}',
          client.params.get('aboutme.name')
        ),
        inline: true
  ...
  ping: {
    info: (client, message) => {
      return client.extraFunction.getLocalizedCommand(client, message, 'ping')
        .info
    },
  ...
  help: {
    ...
    info: (client, message) => {
      return client.extraFunction.getLocalizedCommand(client, message, 'help')
        .info
    },
    ...
    run: (client, message) => {
      ...
      let command = message.content
        .replace(client.extraFunction.getPrefix(client, message) + 'help', '')
        .trim()
      if (command.length === 0) {
        let commandList = {}
        ...
        let embed = {
          ...
          footer: {
            text: client.extraFunction
              .getLocalizedCommand(client, message, 'help')
...
const onMember = (client, message, args, selector) => {
        ...
        if (message.mentions.channels.array().length !== 1) {
          ...
          client.data.put(
            'serverSpecificSettings.' +
              ...
              select[1] +
              '.channel',
            message.mentions.channels.array()[0].id
          )
          client.data.save()
          message.reply('saved!')
        }
          ...
          if (
            ...
            client.channels
              ...
              .send(
                welcomeBlock.message.replace(
                  /{mention}/gm,
                  '<@' + message.guild.me.user.id + '>'
                )
              )
          } else {
            message.reply(
              "I can't really validate your message if I have nothing to.. y'know."
            )
          }
        }
        break
